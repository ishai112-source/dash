...
interface PensionTableProps {
  ...
  onChange: (entries: PensionEntry[]) => void;
}
...
const columns: ColumnDef[] = [
  { key: "owner", label: "בעלים", type: "select", width: "w-28", options: [{ value: "ישי", label: "ישי" }, { value: "מיכל", label: "מיכל" }] },
  { key: "provider", label: "חברה", type: "text", width: "w-36" },
  { key: "fundType", label: "סוג קופה", type: "select", width: "w-32", options: [{ value: "פנסיה", label: "פנסיה" }, { value: "השתלמות", label: "השתלמות" }, { value: "גמל", label: "גמל" }] },
...
const PensionTable = ({ entries, onChange }: PensionTableProps) => {
  const addEntry = () => {
  ...
  const updateEntry = (id: string, field: keyof PensionEntry, value: string | number) => {
  ...
  const renderCell = (entry: PensionEntry, col: ColumnDef) => {
    ...
    <Card className="border-border bg-card">
      <CardHeader className="flex flex-row items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="font-display text-xl">מסלולי השקעה</CardTitle>
        </div>
              ...
              {entries.map((entry) => {
                return (
                  <TableRow key={entry.id}>
                    ...
                    <TableCell>
                      <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-destructive" onClick={(e) => { e.stopPropagation(); deleteEntry(entry.id); }}>
